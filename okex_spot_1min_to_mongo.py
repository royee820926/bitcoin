...
class SpotThread(threading.Thread):
    """
    多线程请求，写入数据
    """
    ...
    def spider(self, start_stamp='', end_stamp=''):
        ...
        try:
            ...
            if start_stamp != '' and end_stamp != '':
                ...
                start_stamp = ''
                end_stamp = ''
            kline_data = SpotApi.get_kline(instrument_id, start=start_stamp, end=end_stamp)
...
for coin_name in spot_coin_type:
    collection_dict[coin_name] = get_spot_collection(coin_name)
...
################################################
# 测试单个
# spot_obj = SpotThread(1, 'BTC-USDT', 1)
# spot_obj.run()
# exit(1)
################################################
...
for coin_name in spot_coin_type:
    ...
    ######################
    thread_index += 1
    thread_dict[coin_name] = SpotThread(thread_index, coin_name, thread_index)
    # 设置守护线程
